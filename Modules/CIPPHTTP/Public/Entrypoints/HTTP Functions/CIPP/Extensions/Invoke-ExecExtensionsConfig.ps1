function Invoke-ExecExtensionsConfig {
    ...
    $Results = try {
        # CIPP-hardened: NinjaOne is unsupported. Do not validate, save, or vault NinjaOne credentials.
        if ($Body.PSObject.Properties.Name -contains 'NinjaOne') {
            $Body.PSObject.Properties.Remove('NinjaOne')
        }
